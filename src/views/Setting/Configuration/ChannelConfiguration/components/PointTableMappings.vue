  ...
  <div class="voltage-class point-table">
    ...
    <div v-if="props.viewMode === 'mappings'" class="table-action-controls">
      ...
      <template v-else-if="props.showActions">
        <span v-if="importedFileName" class="imported-file-name">{{
          importedFileName
        }}</span>
        <el-button type="primary" @mousedown="handleImportClick"
          >Import</el-button
        >
      </template>
    </div>
        ...
        >
          <template #default="{ row }">
            <div class="point-table-cell-wrapper">
                  ...
                  <div class="inline-edit-container">
                    <el-input
                      ...
                      :model-value="
                        col.getValue
                          ? col.getValue(row)
                          : String((row.protocol_mapping as any)[col.key] ?? '')
                      "
                      ...
                      @input="
                        (val: any) =>
                          col.onInput
                            ? col.onInput(row, String(val))
                            : col.onChange && col.onChange(row)
                      "
                    />
                    <el-select
                      size="small"
                    ...
                    >
                      <el-option
                        v-for="option in col.getOptions
                          ? col.getOptions(row)
                          : []"
                        :key="option.value"
                        :label="option.label"
              ...
              <span v-else :class="getFieldClass(row, col.fieldClassKey)">{{
                col.display(row)
              }}</span>
              <div
                v-if="
                  props.isEditing && getMappingFieldError(row, col.errorKey)
                "
                class="field-error"
              >
                {{ getMappingFieldError(row, col.errorKey) }}
...
watch([signalNameFilter, () => props.editFilters], () => {
  currentPage.value = 1;
});
...
const { getRowClass, getFieldClass, hasChanges } =
  useRowStatusHelpers<PointInfo>({
    listRef: editPoints,
    isEditing: () => props.isEditing,
    getBaseRowClass: (row) =>
      `row-status-${(row as any).rowStatus || "normal"}`,
  });
...
const {
  ...
  getFieldsForRow: () => {
    ...
    if (props.channelProtocol === "di_do") return ["gpio_number"];
    return [
      "slave_id",
      "function_code",
      "register_address",
...
watch(
  ...
  (editing) => {
    if (editing && Array.isArray(editPoints.value)) {
      ...
      const baseline = (originalPointsList.value as PointInfo[]).map(
        (item: PointInfo) => clonePointForEdit(item),
      );
...
// }
const getMappingFunctionCodeLabel = (fc: number | undefined) =>
  fc == null ? "" : String(fc);
...
const getMappingByteOrderOptionsSafe = (row: PointInfo) =>
  Array.from(getMappingByteOrderOptions(row)) as Array<{
    label: string;
    value: any;
  }>;
...
const onMappingRegisterAddressInput = (item: PointInfo, str: string) => {
  ...
  if (trimmed === "") {
    ...
    if (digits === "") {
      ...
      item.protocol_mapping.register_address = Math.min(
        65535,
        Math.max(0, num),
      );
    }
...
const getEditedData = () => {
  ...
  editPoints.value.forEach((item) => {
    ...
    const normIntOrNull = (v: any) =>
      v === "" || v === null || v === undefined ? null : Number(v);
    const normStrOrNull = (v: any) =>
      v === "" || v === null || v === undefined ? null : String(v);
    if (props.channelProtocol === "can") {
      const m = item.protocol_mapping as any;
      if (changed.includes("mapping_can_id"))
        data.can_id = normStrOrNull(m.can_id);
      if (changed.includes("mapping_can_offset"))
        data.byte_offset = normIntOrNull(m.byte_offset);
      if (changed.includes("mapping_can_bit_position"))
        data.bit_position = normIntOrNull(m.bit_position);
      if (changed.includes("mapping_can_bit_length"))
        data.bit_length = normIntOrNull(m.bit_length);
      if (changed.includes("mapping_can_data_type"))
        data.data_type = normStrOrNull(m.data_type);
    } else if (props.channelProtocol === "di_do") {
      ...
      if (changed.includes("mapping_register_address"))
        data.register_address = normIntOrNull(
...
const handleExport = async () => {
  ...
  const filename = buildChannelCsvFilename(
    String(injectedChannelName?.value || ""),
    [tabNames[props.pointType], "mapping", getTimestampCompact()],
  );
    ...
    return Array.isArray(editPoints.value)
      ? editPoints.value.some(
          (p: any) =>
            ...
            (p &&
              p.rowStatus !== "deleted" &&
              !validateMappingValidity(p as PointInfo)),
        )
      : false;
  },
  getInvalidDetails: () => {
    ...
    (editPoints.value || []).forEach((p: any) => {
      ...
      const errs =
        (p.fieldErrors &&
          Object.entries(p.fieldErrors).filter(
            ([, v]: [string, unknown]) => v,
          )) ||
        [];
      if (errs.length > 0) {
        const id = p.point_id ?? "-";
        const name = p.signal_name
          ? ` (${String(p.signal_name).slice(0, 20)}${String(p.signal_name).length > 20 ? "..." : ""})`
          : "";
