<template>
  <template v-for="side in SIDES" :key="side.id">
    <Handle type="source" :position="side.position" :id="`${side.id}-source`" connectable
      :style="handleStyle(side.id)"
      :class="handleClass(side.id, 'source')" />
    <Handle type="target" :position="side.position" :id="`${side.id}-target`" connectable
      :style="handleStyle(side.id)"
...
const props = withDefaults(
  defineProps<{ viewMode?: boolean; borderColor?: string }>(),
...
function handleStyle(side: (typeof SIDES)[number]['id']) {
  return {
    ...
    top: side === 'top' ? '1px' : ['left', 'right'].includes(side) ? '50%' : 'auto',
    right: side === 'right' ? '1px' : 'auto',
    bottom: side === 'bottom' ? '1px' : 'auto',
    left: side === 'left' || ['top', 'bottom'].includes(side) ? (side === 'left' ? '1px' : '50%') : 'auto',
...
const connectedHandleIds = computed(() => new Set(
  connections.value.flatMap((connection) => [connection.sourceHandle, connection.targetHandle]
...
function handleClass(side: string, type: 'source' | 'target') {
  ...
  return [
    ...
    props.viewMode
      ? (connectedHandleIds.value.has(id) ? 'node-connection-handle--connected' : '')
      : 'node-connection-handle--edit',
  ]
}

function toViewHandleId(handle: string) {
  const match = handle.match(/^(top|right|bottom|left)(?:-[^-]+)*-(source|target)$/)
