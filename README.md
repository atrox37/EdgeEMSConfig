...
- 🪟 **全局标题栏**：`TitleBar` 在 `App.vue` 中挂载，所有路由页面共用（含登录与首次向导）
- 🧭 **首次网关向导**：未完成向导前访问任意路由会先被重定向到 `/setup`（逻辑见 `src/router/guard.ts`，标记键 `monarch-first-gateway-setup-seen`）；跳过或安装成功后会写入标记。登录页 **Initialize Project** 会跳转到 `/setup`（与向导共用 `runGatewayInitInstall` 安装流程）。初始化页在描述文字下方提供简短提示（与正文仅颜色区分）及 **Skip to login**；登录失败且判断为网关未就绪时会跳转至 `/setup`，也可随时从登录页 **Initialize Project** 进入
- 🖥️ 原生桌面应用体验
- ⚡ 快速启动和响应
- 🔒 系统级权限访问
- 📱 现代化UI界面
- 🎯 配置界面风格一致（通道与设备点表统一视图样式）
- 🪟 通道详情/新增改为弹窗编辑，参数表单组件复用并统一主题文字色
- 🧽 全局滚动条样式统一（更高透明度、8px宽度、圆角优化）
- 🆔 新增通道支持ID自动/手动填写（手动时提交 channel_id）
- 📐 通道详情表单输入/选择控件宽度统一，单行容纳2-3项
- ✅ 新增通道默认启用，参数区与基础信息保持两列布局
- 🔽 标题栏用户下拉箭头随菜单展开旋转
- 🧭 设备实例点位路由独立页面（与通道点表一致的交互体验）
- 🧩 设备点位/路由表切换为普通表格与分页，导入导出规则对齐通道
- 🧪 点位/路由校验与CSV解析抽离到 validators/schemas，便于维护
- ⬆️ 系统配置升级上传与状态轮询分离，完成状态才结束流程
- 🌐 系统配置新增有线网络设置弹窗（LAN1-LAN4，支持 DHCP/静态 IP、DNS 保存与应用）
- 🗄️ 系统配置新增 Storage 页面（支持 PostgreSQL 参数保存、连通性测试、重连与状态展示）
- 🎨 全局消息提示样式统一为主题色系
- 🧭 规则链监控模式支持快照与编辑态切换
- 🔗 规则链详情支持通过 URL 参数 id 直接打开
- 🔌 WebSocket 根据用户配置地址初始化并在登录后连接
- 🎨 规则链编辑器视图/编辑态样式对齐项目主题
- 🧩 规则链编辑器侧栏/折叠面板细化样式与圆角规则
- 🧷 规则链编辑器支持侧栏标题与拖拽优化
- 🎛️ 规则链折叠面板主题/副主题色与拖拽交互增强
- 🧲 规则链拖拽交互兼容浏览器/运行时默认行为
- 🧾 规则卡片编辑弹窗主题与变量区自适应
- 🧠 规则卡片变量提示与小屏压缩交互
- 🪟 Tauri 原生窗口 resize 监听（弹窗响应式可靠触发）
- 🛠️ 完整的开发工具链
...
2. **再次体验首次向导**: 在应用内打开开发者工具，执行 `localStorage.removeItem('monarch-first-gateway-setup-seen')` 后刷新，会重新进入 `/setup`
3. **热重载**: 开发模式下支持前端代码热重载
4. **系统权限**: 某些功能可能需要系统权限，请根据提示授权
5. **调试**: 可以使用浏览器开发者工具调试前端代码
